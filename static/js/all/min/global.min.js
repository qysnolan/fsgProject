...
return cookieValue;}
window.TeacherEval={};window.TeacherEval.init=function(){};window.TeacherEval.UI={};window.TeacherEval.UI.ArrowDirections={down:"s",up:"n",left:"w",right:"e"};window.TeacherEval.UI.Button={};(function(Button,UI){Button.style=function($element,arrowFacing){if($element.length>1){$element.each(function(){Button.style($(this),arrowFacing);});}else if($element.length==1){if($element.hasClass("ui-button")==false){if(!$element.is("input")){var $textSpan=$("<span />");$textSpan.addClass("ui-button-text");$textSpan.html($element.html());$element.html($textSpan);}
$element.attr("role","button");$element.addClass("ui-button ui-widget ui-state-default ui-corner-all");$element.bind("mouseenter focus",this.handleButtonMouseEnter);$element.bind("mouseleave blur",this.handleButtonMouseLeave);$element.bind("mousedown",this.handleButtonMouseDown);$element.bind("mouseup",this.handleButtonMouseUp);if(arrowFacing){if(arrowFacing in UI.ArrowDirections){var $arrowSpan=$("<span />");$element.addClass("ui-button-text-icon-secondary");$arrowSpan.addClass("ui-button-icon-secondary ui-icon");$arrowSpan.addClass("ui-icon-triangle-1-"+UI.ArrowDirections[arrowFacing]);$element.append($arrowSpan);}else{console.log("Invalid direction specified for button arrow:",arrowFacing);}}else{$element.addClass("ui-button-text-only");}}}};Button.changeArrow=function($element,arrowFacing){if($element.length>1){$element.each(function(){Button.changeArrow($(this),arrowFacing);});}else if($element.length==1){if(arrowFacing in UI.ArrowDirections){var $arrowSpan=$("<span />");if($element.hasClass("ui-button-text-icon-secondary")){$element.find(".ui-button-icon-secondary").remove();}
$element.addClass("ui-button-text-icon-secondary");$arrowSpan.addClass("ui-button-icon-secondary ui-icon");$arrowSpan.addClass("ui-icon-triangle-1-"+UI.ArrowDirections[arrowFacing]);$element.append($arrowSpan);}else{console.log("Invalid direction specified for button arrow:",arrowFacing);}}}
Button.handleButtonMouseEnter=function(event){var $this=$(this);$this.addClass("ui-state-hover");};Button.handleButtonMouseLeave=function(event){var $this=$(this);if($this.is(":focus")==false){$this.removeClass("ui-state-hover ui-state-active");}};Button.handleButtonMouseDown=function(event){var $this=$(this);$this.addClass("ui-state-active");};Button.handleButtonMouseUp=function(event){var $this=$(this);$this.removeClass("ui-state-active");};})(window.TeacherEval.UI.Button,window.TeacherEval.UI);window.TeacherEval.UI.MultiSelect={};(function(MultiSelect){MultiSelect.style=function($element,options){if($element.length>1){$element.each(function(){MultiSelect.style($(this),options);});}else if($element.length==1){if(!$element.data("multiselect")){var $container=$("<div />"),$selectableContainer=$("<div />"),$selectableList=$("<ul />"),$selectedContainer=$("<div />"),$selectedList=$("<ul />"),containerName="multiselect-"+$element.attr("name"),containerHeight=($element.attr("size")*25)+"px",$selectOptions=$element.find("option");if(options){if("selectableHeader"in options){var $selectableHeader=$("<div />");$selectableHeader.addClass("ui-widget-header ui-multiselect-header");$selectableHeader.html(options.selectableHeader);$selectableContainer.append($selectableHeader);}
if("selectedHeader"in options){var $selectedHeader=$("<div />");$selectedHeader.addClass("ui-widget-header ui-multiselect-header");$selectedHeader.html(options.selectedHeader);$selectedContainer.append($selectedHeader);}}
$element.data("multiselect",containerName);$element.css('position','absolute').css('left','-9999px');$container.data("original",$element);$container.attr("id",containerName);$container.addClass("ui-multiselect");$selectableContainer.addClass("ui-multiselect-selectable");$selectableList.addClass("ui-multiselect-list");$selectableList.height(containerHeight);$selectableList.on("mouseenter","li",this.handleOptionMouseEnter);$selectableList.on("mouseleave","li",this.handleOptionMouseLeave);$selectableList.on("click","li",this.handleOptionClick);$selectableContainer.append($selectableList);$container.append($selectableContainer);$selectedContainer.addClass("ui-multiselect-selection");$selectedContainer.css("minHeight",containerHeight);$selectedList.addClass("ui-multiselect-list");$selectedList.height(containerHeight);$selectOptions.each(function(){var $optionElement=$("<li />");$optionElement.addClass("ui-multiselect-option");$optionElement.data("value",$(this).val());$optionElement.text($(this).text());if($(this).is(":selected")){$optionElement.data("selected",true);$selectedList.append($optionElement);}else{$optionElement.data("selected",false);$selectableList.append($optionElement);}});$selectedList.on("mouseenter","li",this.handleOptionMouseEnter);$selectedList.on("mouseleave","li",this.handleOptionMouseLeave);$selectedList.on("click","li",this.handleOptionClick);$selectedContainer.append($selectedList);$container.append($selectedContainer);$element.after($container);}}};MultiSelect.handleOptionMouseEnter=function(event){var $this=$(this);$this.addClass("ui-multiselect-option-hover");};MultiSelect.handleOptionMouseLeave=function(event){var $this=$(this);if($this.is(":focus")==false){$this.removeClass("ui-multiselect-option-hover");}}
MultiSelect.handleOptionClick=function(event){var $this=$(this),$root=$this.parents(".ui-multiselect"),$original=$root.data("original")
originalValues=$original.val()||[],$selectableList=$root.children(".ui-multiselect-selectable").children("ul"),$selectedList=$root.children(".ui-multiselect-selection").children("ul");if($this.data("selected")){$this.data("selected",false);originalValues.splice(originalValues.indexOf($this.data("value")),1);}else{originalValues.push($this.data("value"));$this.data("selected",true);}
$this.removeClass("ui-multiselect-option-hover");$original.val(originalValues);$this.detach();if($this.data("selected")==false){$selectableList.append($this);}else{$selectedList.append($this);}}})(window.TeacherEval.UI.MultiSelect);window.TeacherEval.Menu={};(function(Menu,UI){Menu.init=function(){var buttons=[],$navButtons,buttonId=0;this.$nav=$("#nav-list");this.$nestedNavs=this.$nav.find(".nav-nested-list");this.$sub=$(".secondary");this.$subNav=$(".secondary-list");$navButtons=this.$nav.children("li");$navButtons.each(function(index,navButton){var $navButton=$(navButton),tempObject={id:buttonId++},$anchor=$navButton.find("a").first(),$subMenu=$navButton.find("ul");tempObject.$object=$anchor;tempObject.title=$anchor.text();tempObject.page=$anchor[0].href;if($subMenu.length>0){tempObject.subMenu=[];tempObject.$subMenu=$subMenu;$subMenu.find("a").each(function(index,subAnchor){var tempSubObject={id:buttonId++};tempSubObject.title=subAnchor.innerText;tempSubObject.page=subAnchor.href;tempSubObject.$object=$(subAnchor);tempObject.subMenu.push(tempSubObject);});}
buttons.push(tempObject);});this.buttons=buttons;this.$subMenus=[];this.$shownMenu=false;this.buildPrimaryMenu();this.buildSubMenus();};Menu.buildPrimaryMenu=function(){var buttonLength=this.buttons.length;this.$nav.children().remove();this.$nav.addClass("nav-list");for(i=0;i<buttonLength;i++){var button=this.buttons[i],$button=button.$object;$button.data("id",button.id);if(typeof button.subMenu=="object"){UI.Button.style($button,"down");$button.data("subMenu",i);$button.click(this.handleSubMenuClick);}else{UI.Button.style($button);$button.data("subMenu",false);}
this.$nav.append($button);}};Menu.buildSubMenus=function(){var buttonLength=this.buttons.length;for(i=0;i<buttonLength;i++){var button=this.buttons[i];if(typeof button.subMenu=="object"){button.$object.button("option","icons",{secondary:'ui-icon-triangle-1-s'});var subMenu=button.subMenu,subLength=subMenu.length,$subMenu=$("<ul />");$subMenu.data("id",i);for(s=0;s<subLength;s++){var subButton=subMenu[s],$subItem=$("<li/>");$subItem.html(subButton.$object);$subMenu.append($subItem);}
this.$subMenus.push($subMenu);}else{this.$subMenus.push(false);}}}
Menu.handleSubMenuClick=function(event){event.preventDefault();Menu.$sub.outerWidth($(window).outerWidth());var $button=$(this),buttons=Menu.buttons,button=buttons[$button.data("id")];if(Menu.$shownMenu===false){Menu.showSubMenu($button.data("subMenu"));$button.addClass("nav-button-active");UI.Button.changeArrow($button,"up");}else if(parseInt(Menu.$shownMenu.data("id"))==parseInt($button.data("subMenu"))){Menu.hideSubMenu();$button.removeClass("nav-button-active");UI.Button.changeArrow($button,"down");}else{Menu.switchSubMenu($button.data("subMenu"));UI.Button.changeArrow($(".nav-button-active"),"down");$(".nav-button-active").removeClass("nav-button-active");$button.addClass("nav-button-active");UI.Button.changeArrow($button,"up");}};Menu.showSubMenu=function(id){var $subMenu=Menu.$subMenus[id];Menu.$subNav.html($subMenu.html());UI.Button.style(Menu.$subNav.find("a"));Menu.$shownMenu=$subMenu;Menu.$sub.slideDown("slow");};Menu.hideSubMenu=function(){Menu.$shownMenu=false;Menu.$sub.slideUp("slow");};Menu.switchSubMenu=function(id){var $subMenu=Menu.$subMenus[id];Menu.$shownMenu=$subMenu;Menu.$subNav.fadeTo("400",0,function(){$(this).html($subMenu.html());UI.Button.style($(this).find("a"));$(this).fadeTo("400",1);});};})(window.TeacherEval.Menu,window.TeacherEval.UI);$(document).ready(function(){var elem=document.createElement('input');elem.setAttribute('type','date');elem.value='foo';if(!(elem.type=='date'&&elem.value!='foo')){$('input[type=date]').datepicker({dateFormat:"yy-mm-dd"});}
$(".container").addClass("ui-widget ui-widget-content");window.TeacherEval.UI.Button.style($(".button"));$(".buttonset").buttonset();$(".buttonset :checked").button("refresh");$(".sectioned").each(function(){var $headers=$(this).children(":header");$headers.each(function(){if($(this).data("show")===false){$(this).addClass("ui-accordian-header ui-helper-reset ui-corner-all ui-state-active");$content=$(this).next("div");$content.addClass("ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom ui-content-active");$content.hide();$(this).prepend("<span style=\"display: inline-block; margin-right: 5px;\" class=\"ui-icon ui-icon-triangle-1-e\"></span>");}else{$(this).data("show",true);$(this).addClass("ui-accordian-header ui-helper-reset ui-corner-top ui-state-active");$content=$(this).next("div");$content.addClass("ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom ui-content-active");$(this).prepend("<span class=\"sectioned-icon ui-icon ui-icon-triangle-1-s\"></span>");}});$headers.click(function(){if($(this).data("show")){$head=$(this);$content=$head.next("div");$content.slideUp("400",function(){$head.data("show",false).removeClass("ui-corner-top").addClass("ui-corner-all");$head.children("span").removeClass("ui-icon-triangle-1-s").addClass("ui-icon-triangle-1-e");});}else{$content=$(this).next("div");$content.slideDown("400",function(){});$(this).data("show",true).removeClass("ui-corner-all").addClass("ui-corner-top");$(this).children("span").removeClass("ui-icon-triangle-1-e").addClass("ui-icon-triangle-1-s");}});});$(document).delegate(".prompt","click",function(e){if(!confirm($(this).data("prompt").replace(/\[n\]/g,"\n"))){e.preventDefault();}});$(".notice").click(function(){window.location.hash="no-message";$(this).fadeOut();});$("form.validate").submit(function(event){$(".notice").fadeOut();$form=$(this);$(this).find("input[required]").each(function(){if(this.type=="email"){if(!$(this).val().match(/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/i)){$(this).addClass("error");$form.find("div.invalid").html("You must submit a valid email address").fadeIn();event.preventDefault();}}
if($(this).val()===""){$(this).addClass("error");$form.find("div.invalid").html("The required fields must be filled in").fadeIn();event.preventDefault();}});});$("form.validate").on("change","input.error",function(){$(this).removeClass("error");});!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");});(function(i,V,l,n){var j=function(e){function o(a,b){var c=j.defaults.columns,d=a.aoColumns.length,c=i.extend({},j.models.oColumn,c,{sSortingClass:a.oClasses.sSortable,sSortingClassJUI:a.oClasses.sSortJUI,nTh:b?b:l.createElement("th"),sTitle:c.sTitle?c.sTitle:b?b.innerHTML:"",aDataSort:c.aDataSort?c.aDataSort:[d],mDataProp:c.mDataProp?c.oDefaults:d});a.aoColumns.push(c);if(a.aoPreSearchCols[d]===n||null===a.aoPreSearchCols[d])a.aoPreSearchCols[d]=i.extend({},j.models.oSearch);else if(c=a.aoPreSearchCols[d],c.bRegex===n&&(c.bRegex=!0),c.bSmart===n&&(c.bSmart=!0),c.bCaseInsensitive===n)c.bCaseInsensitive=!0;r(a,d,null)}function r(a,b,c){b=a.aoColumns[b];c!==n&&null!==c&&(c.sType!==n&&(b.sType=c.sType,b._bAutoType=!1),i.extend(b,c),p(b,c,"sWidth","sWidthOrig"),c.iDataSort!==n&&(b.aDataSort=[c.iDataSort]),p(b,c,"aDataSort"));b.fnGetData=W(b.mDataProp);b.fnSetData=ta(b.mDataProp);a.oFeatures.bSort||(b.bSortable=!1);!b.bSortable||-1==i.inArray("asc",b.asSorting)&&-1==i.inArray("desc",b.asSorting)?(b.sSortingClass=a.oClasses.sSortableNone,b.sSortingClassJUI=""):b.bSortable||-1==i.inArray("asc",b.asSorting)&&-1==i.inArray("desc",b.asSorting)?(b.sSortingClass=a.oClasses.sSortable,b.sSortingClassJUI=a.oClasses.sSortJUI):-1!=i.inArray("asc",b.asSorting)&&-1==i.inArray("desc",b.asSorting)?(b.sSortingClass=a.oClasses.sSortableAsc,b.sSortingClassJUI=a.oClasses.sSortJUIAscAllowed):-1==i.inArray("asc",b.asSorting)&&-1!=i.inArray("desc",b.asSorting)&&(b.sSortingClass=a.oClasses.sSortableDesc,b.sSortingClassJUI=a.oClasses.sSortJUIDescAllowed)}
